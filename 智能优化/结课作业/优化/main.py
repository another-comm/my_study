...
def run_hybrid_optimization():
    ...
    # PSO 阶段：无需梯度，使用 Numba 加速
    with torch.no_grad():
        for it in range(CFG.MAX_ITER):
            scores = []
            # 粒子群评估（批量，无梯度，减少 host<->device 往返）
    ...
    if CFG.ENABLE_HYBRID:
        ...
        use_amp = (CFG.DEVICE.type == "cuda" and CFG.ENABLE_AMP)
        scaler = GradScaler('cuda', enabled=use_amp) if use_amp else None

        # 在梯度优化开始前记录一次当前损失，使曲线从 PSO 阶段平滑接上
        if use_amp:
            with autocast('cuda', enabled=True):
                init_loss = problem.get_loss(model)
        else:
            init_loss = problem.get_loss(model)
        history.append(init_loss.item())
