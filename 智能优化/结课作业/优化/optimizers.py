...
class BaseOptimizer:
    def __init__(self, dim, pop_size, max_iter):
        ...
        # 初始化位置 [-range, range] 范围，全部放在目标设备
        init_range = CFG.PSO_POS_INIT_RANGE
        self.particles = torch.empty((pop_size, dim), device=CFG.DEVICE).uniform_(-init_range, init_range)
...
class EntropyPSO(StandardPSO):
    def _calculate_entropy(self):
        """
        使用直方图近似计算种群在各维度上的香农熵，衡量多样性。
        熵越大 -> 个体分布越均匀、多样性越好。
        """
        # 归一化到 [0, 1] 区间，避免某一维尺度过大主导熵值
        ...
        # 对每一个维度计算 1D 直方图熵，并取平均
        for d in range(self.dim):
            # hist: [num_bins], 所有 bin 计数之和等于 pop_size
            hist = torch.histc(norm_p[:, d], bins=num_bins, min=0.0, max=1.0)
            total = hist.sum()
            if total <= 0:
                continue
